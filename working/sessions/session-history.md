...
- **Data**: 16 Agosto 2026 (CEST)
- **Obiettivo**: Dimostrare che la produzione immagini può iniziare senza una
  `DreamStory` narrativa completa come causa.
- **Baseline**: commit `e21fddb`; branch
  `feature/brain-dream-causality-experiment`.
- **Implementazione**: il controller combina materiale già disponibile e usa
  un solo task Qwen `scene`; il `VisualPlan` viene adattato al contenitore
  legacy soltanto dopo essere stato prodotto. `Psichedel`, buffer, timeline,
  scheduler e renderer restano invariati.
- **Memoria**: nessun prompt tecnico o episodio sperimentale viene consolidato
  come ricordo onirico; origine e presente percettivo restano attivi.
- **Osservazione reale**: 3 `scene`, 0 `story`, 0 traduzioni, 0 `memo`; Qwen ha
  copiato input astratti, reintrodotto un protagonista e propagato ripetizioni
  degenerative. La causalità narrativa è separata, la qualità non è ancora
  sufficiente.
- **Taratura successiva**: rimossi i divieti che sopprimevano associazioni o
  frammenti narrativi spontanei e il minimo forzato di token. Il moto di
  coscienza è ora riconoscibile nel materiale come influenza non prescrittiva;
  resta una sola chiamata Qwen `scene` per episodio.
- **Validazione**: 50 file / 299 test, typecheck, lint, diff-check e build
  Electron arm64 verdi; esecuzione live completata e architettura fermata.

### `SESSION-2026-08-16-24`
- **Data**: 16 Agosto 2026 (CEST)
- **Obiettivo**: Rendere FilterPsiche più presente e più dinamico.
- **Regia**: presenza garantita nei quattro renderer di ogni episodio in
  “Tutti per storia”; durata aumentata del 50% nella rotazione automatica.
- **Materia**: intensificate inversione, alternanza cromatica, contrasto e
  micro-slice secondo bande e beat, senza moto globale o attività nel silenzio.
- **Costo**: invariati FPS, risoluzione, buffer, massimo di sette slice,
  `lowPowerMode` e gestione della pressione risorse.
- **Validazione**: 50 file / 301 test, typecheck, lint e diff-check verdi;
  verifica percettiva fullscreen consigliata.

### `SESSION-2026-08-16-25`
- **Data**: 16 Agosto 2026 (CEST)
- **Obiettivo**: Eliminare il fermo percepito dopo quattro immagini e gli
  stalli dominanti durante il denoising.
- **Diagnosi live**: Output a circa 12,06 GB RSS, processo GPU 3,2–3,7 GB e
  oltre 100% CPU; 18 gap RAF da 240–347 ms, un vuoto IPC di 19,86 s e ultima
  inferenza a 30,8 s. I renderer Canvas non erano il collo di bottiglia.
- **Produzione**: refill story-cycle non più sospeso dal primo giro; stessa
  finestra 120/90 s delle altre modalità, quindi nuovo avvio dopo 30 s e ciclo
  senza limite di episodi.
- **Denoising**: profilo live standard 448×256/8 step, yield di 48 ms fra step,
  sessioni SD residenti anche in low power e Qwen `scene` residente su WASM.
- **Regia**: FilterPsiche, Materia Morph e Vector Morph persistono casualmente
  2–4 immagini; Psycho2D resta casuale e singolo.
- **Validazione**: 50 file / 302 test, typecheck, lint, diff-check, build Vite,
  Electron, ZIP e DMG arm64 verdi. Prova live da processo pulito pendente.

### `SESSION-2026-08-16-26`
- **Data**: 16 Agosto 2026 (CEST)
- **Obiettivo**: Correggere la regressione che impediva la generazione e la
  ripetizione semantica delle immagini.
- **Causa verificata**: Qwen su WASM ha prodotto soltanto 25 token in circa 35
  secondi ed è arrivato al timeout; il worker veniva terminato e ricaricava 769
  MB al tentativo seguente, senza raggiungere Psichedel.
- **Correzione**: Qwen `scene` torna su WebGPU e viene rilasciato prima di SD.
  Se fallisce o produce output invalido, Brain usa il materiale già disponibile
  senza una seconda chiamata Qwen.
- **Concatenazione**: ogni prompt comprende osservazione corrente, stimolo
  associato ruotato e residuo dell'osservazione precedente.
- **Prova live**: pipeline ripartita; primo raster standard 448×256/8 step in
  7,65 s, secondo raster completato, prompt differenti confermati nei log.
- **Prova ciclo**: primo buffer completato 4/4; esattamente 30 s dopo Brain ha
  avviato il refill e la seconda produzione ha raggiunto almeno 3/4 durante la
  verifica. I prompt osservati sono differenti e concatenano tre provenienze.
- **Validazione**: 50 file / 305 test, typecheck, lint, diff-check e build
  runtime Electron verdi. ZIP completato; DMG finale fallito in `hdiutil`, non
  nel codice o nel packaging dell'applicazione.

### `SESSION-2026-08-16-27`
- **Data**: 16 Agosto 2026 (CEST)
- **Obiettivo**: Rimuovere la riga orizzontale visibile al centro di
  FilterPsiche.
- **Causa**: con un numero dispari di micro-slice, una posizione cadeva
  deterministicamente al 50% dell'altezza del canvas.
- **Correzione**: esclusa la fascia centrale 46–54%; conservate le slice locali
  superiori e inferiori, senza cambi di camera, beatmatch o budget.
- **Validazione**: test mirato 6/6, typecheck, lint e diff-check verdi.
